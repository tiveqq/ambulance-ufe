...
@Component({
  tag: 'karunnyi-ambulance-wl-editor',
  styleUrl: 'karunnyi-ambulance-wl-editor.css',
  shadow: true,
})
export class KarunnyiAmbulanceWlEditor {
  ...
  private async assumedEntryDateAsync(): Promise<Date> {
    try {
      ...
      const lastPatientOut = (await response.value())
        ...
        .reduce((acc: number, value: number) => Math.min(acc, value), 0);
